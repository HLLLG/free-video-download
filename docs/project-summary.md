...
> 最近更新：2026-05-02（同上；新增 SQLite 会员库、Stripe Checkout、会员密钥恢复、Pro 权益：4K / 不限次总结 / 最长 120 分钟总结）
...
- 限流策略：按 IP 内存限流，环境变量 `SUMMARY_DAILY_LIMIT_PER_IP` 控制每日次数；**默认值为 `0` 表示不限制**（便于测试与 MVP），生产可设为 `5` 等正整数。**有效 Pro 会员**（请求头 `X-Membership-Key` 对应用户未过期）**跳过每日次数限制**。单视频最长：免费用户由 `SUMMARY_MAX_DURATION_SECONDS` 限制（默认 40 分钟）；Pro 用户由 `PRO_SUMMARY_MAX_DURATION_SECONDS` 限制（默认 120 分钟）。
