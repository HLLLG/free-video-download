...
- 免费限额：AI 总结次数由环境变量 `SUMMARY_DAILY_LIMIT_PER_IP` 控制（默认 `3` 次/天；设置为 `<=0` 可关闭限额）；免费用户单视频总结最长默认 40 分钟。
...
- **封面**：修复 `/api/thumbnail` 上游请求与 FastAPI `Request` 同名冲突导致的 500，恢复 CDN 封面。
- **B 站**：网页风控（如 `412`）时，`backend/app/bilibili.py` 使用 Open API + ffmpeg 兜底解析与下载，失败再回退 `yt-dlp`。
- **AI 总结限额**：`SUMMARY_DAILY_LIMIT_PER_IP` 默认 **3 次/天**（`<=0` 关闭限额）；Pro 跳过日限额。
- **升级 Pro**：弹窗同时展示月卡/年卡与权益；免费次数用尽时专属文案；限额场景下 AI 总结区可一键打开升级。

详见 [`docs/project-summary.md`](docs/project-summary.md) §11。
